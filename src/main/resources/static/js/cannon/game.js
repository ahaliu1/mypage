...
Game.prototype.areaDispatch = function (area, event) {
    ...
    for (var i = this.actors.length - 4; i > -1; i--) {
      
        var distance =Math.sqrt(Math.pow(this.actors[i].x - area.left,2) + Math.pow(this.actors[i].y - area.top,2));
        var safeDistance = (Math.sqrt(Math.pow(area.width,2) + Math.pow(area.height,2)) + Math.sqrt(Math.pow(this.actors[i].width,2) + Math.pow(this.actors[i].height,2))) / 2;

        if (distance < safeDistance-10) {
            this.directDispatch(event, this.actors[i]);
        }
...
Game.prototype.dispatchToAll = function (event) {
    ...
    for (var i = this.actors.length - 1; i > -1; i--) {
        this.actors[i].deliverEvent(event, this);
    }
}
...
Game.prototype.clear = function () {
    this.context.clearRect(0, 0, this.canvans.width, this.canvans.height);
}
